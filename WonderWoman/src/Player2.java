...
class Player2 implements Comparable<Player> {
    ...
    public static final int PRIORITE_ATTACK = 100;
    ...
    public static double[] centerPoint;
    ...
    public static void main(String args[]) {
        ...
        centerPoint = new double[]{size / 2.0, size / 2.0};
    ...
    private static void countPointsForAction(Action action) {
        ...
        if (action.atype.equals("PUSH&BUILD")) {
            ...
            float movePoints = countMovePoints(action);
...
/*
        //Calculer la distance avec le centre
        float distanceX = Math.abs(newPosition[0] - centerPoint[0]);
        float distanceY = Math.abs(newPosition[1] - centerPoint[1]);
        float floatRes = distanceX > distanceY ? distanceX : distanceY;
        points += (grid.length - floatRes) * 0.1;
*/
        return points;
    }
    ...
    public static void findEnnemiPosition() {
        ...
        for (Iterator<EnnemiPosition> it = oldEnnemiList.iterator(); it.hasNext(); ) {
            ...
            for (Iterator<EnnemiPosition> it1 = cloneEnnemiPositionList.iterator(); it1.hasNext(); ) {
                ...
                } else if (!oldPotentialPositions.isEmpty()) {
